...
/**
 * Audit-tracker bound to {@link TransactionScoped} — one instance per
 * @Transactional invocation. Static counters record how many
 * {@code @PostConstruct} / {@code @PreDestroy} fires occur over the
 * test method, so the test can assert one full lifecycle per tx.
 */
@TransactionScoped
public class TxScopedAuditTracker implements Serializable {
    ...
    /**
     * Force the contextual instance to be created — invoking any
