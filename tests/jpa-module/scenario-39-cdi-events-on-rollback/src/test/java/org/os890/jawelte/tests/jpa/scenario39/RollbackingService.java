...
/** {@code @Transactional} service that throws to force rollback. */
@ApplicationScoped
public class RollbackingService {
    ...
    /** Persist + flush, then throw — interceptor rolls back. */
    @Transactional
    public void persistAndThrow() {
        entityManager.persist(new Marker());
        entityManager.flush();
        throw new RuntimeException("scenario-39: forced rollback");
    }
}
