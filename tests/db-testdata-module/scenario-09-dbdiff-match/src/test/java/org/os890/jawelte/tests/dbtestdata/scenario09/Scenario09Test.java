 ...
 */
package org.os890.jawelte.tests.dbtestdata.scenario09;
...
class Scenario09Test {
    ...
    void openConnection() throws Exception {
        connection = DriverManager.getConnection(
                "jdbc:h2:mem:scenario09;MODE=LEGACY;DB_CLOSE_DELAY=-1", "sa", "");
    ...
    @Test
    void seedThenDiffReturnsSilentlyWhenDatabaseMatchesExpected() {
        String dataset = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
                + "<dataset>"
                + "<CUSTOMER ID=\"1\" NAME=\"Alice\"/>"
                + "<CUSTOMER ID=\"2\" NAME=\"Bob\"/>"
                + "</dataset>";
        DbSeed.forConnection(connection).datasetContent(dataset).cleanInsert().execute();
