...
class Scenario42Test {
    ...
    @Test
    void eachThreadSeesItsOwnTestContextNoLeakageAcrossThreads() throws InterruptedException {
        AtomicReference<Long> threadAId = new AtomicReference<>();
        AtomicReference<Long> threadBId = new AtomicReference<>();

        Thread threadA = new Thread(() -> runSubject(Scenario42SubjectA.class, threadAId));
        threadA.start();
        threadA.join();

        Thread threadB = new Thread(() -> runSubject(Scenario42SubjectB.class, threadBId));
        threadB.start();
        threadB.join();
        ...
        assertThat(threadAId.get()).isNotEqualTo(threadBId.get());
    }

    private static void runSubject(Class<?> subjectClass, AtomicReference<Long> threadIdSink) {
