...
## 2026-05-22 — docs/jta-module.html + listings 02/03 (pending T1..T16)

Fixes applied:
- **T1** (high userfriendly): all priority phrasing rewritten to Jakarta's "lower numeric @Priority wins (ascending sort)" convention. §2.4 names the actual default values (`Integer.MAX_VALUE` for jpa-module fallback, `Integer.MAX_VALUE - 100` for jta-module). §3 paragraph reframed: "@Priority(1) has a lower numeric value, wins the ascending sort". "highest-priority" colloquialism removed.
- **T2** (high userfriendly): §2.4 priority bullet no longer claims "no persistence.xml rewrite". Replaced with explicit "no annotation change in the tests; the persistence.xml transaction-type='JTA' must already be set for JPA to enlist its EntityManager into the JTA transaction (see §1.2)". The contradiction with §1.2's hello-world prose is gone.
- **T6** (medium ordering): TOC expanded from 2 entries to 4. §2 fully reordered with beginner-facing first: §2.1 programmatic UserTransaction (was h3 inside §2.1) · §2.2 @TransactionScoped CDI bean (was h3 inside §2.1) · §2.3 picking a transaction manager (was §2.2) · §2.4 priority-based strategy selection (was §2.1, moved last as mechanism note).
- **T8** (medium userfriendly): Narayana explanation moved out of the listing-02 trailing prose into §2.2 @TransactionScoped CDI bean (where it belongs) with explicit framing — Geronimo provides the TM, Narayana provides the @TransactionScoped CDI context activator that listens to begin/commit/rollback. Listing 02's trailing sentence now just points forward to §2.2.
- **T10** (medium names): listing 03 `PerTxBeanReader` → `TransactionalCaller` via `git mv` + class/method rename; `readIdInsideJtaTx` → `enterTxAndReadBeanId`; matching updates in `PerTxBeanTest` (field, two call sites) + HTML pre-block. Tests pass.
- **T11** (low names): listing 02 Marker entity gains a class-level Javadoc explaining why it diverges from the Customer entity used in listing 01 ("test is about transaction lifecycle, not domain shape"). Rename rejected because the schema in persistence.xml + the EM proxy already point at Marker; the doc gap is what matters.
- **T13** (low userfriendly): §1.3 maven setup now spells out why a JNDI provider is in the list — "JTA TransactionManagers are conventionally published under JNDI names like java:/TransactionManager; jta-module's JndiArtifactBinder publishes Geronimo there at startup and looks the TM up when the strategy needs it."
- **T14** (low userfriendly): §1.2 hello-world preconditions extracted from a prose paragraph into a 3-item bullet list — @ApplicationScoped + @Transactional service, persistence.xml transaction-type="JTA", test method itself NOT @Transactional.
- **T16** (low names): §3 RecordingTransactionManagerProvider paragraph now prepends the teaching device explanation — "A recording wrapper is the simplest way to verify SPI priority wiring — counting every create() call lets the test prove the priority sort picked the custom impl over the default."

## 2026-05-22 — docs/jpa-module.html + listing 04 (pending J3 + J10..J16)

Fixes applied:
- **J3** (high ordering): §2 fully reordered so listings appear in 01, 02, 03, 04, 05 sequence. New order: §2.1 @PersistenceConfig · §2.2 transactions & EM (was 2.3) · §2.3 multi-PU usage (was 2.4) · §2.4 @ReadOnly (was 2.2) · §2.5 transaction events · §2.6 EmfCache. TOC + heading numbers + the §2.2 cross-link to multi-PU updated. No folder renames needed.
- **J10** (low userfriendly): §2.4 @ReadOnly prose now lists the three concrete behaviours under `ResourceLocalTransactionStrategy` — (1) rollback-only marker when wrapped by @Transactional, (2) per-method cleanup skipped, (3) Hibernate session-level `setDefaultReadOnly(true)`. The vague "connection mode tuned for reads" phrase is gone.
- **J11** (low userfriendly): §2.3 multi-PU dropped the implementation-detail `JpaActivePersistenceUnits` ThreadLocal reference. The paragraph keeps the user-visible mechanics (routing proxy, lazy join, commit/rollback closing) but no longer names internal helpers.
- **J13** (low names): listing 04 `TransactionAuditLog` → `TransactionTimeline` via `git mv` + class rename; field `auditLog` → `timeline`; method `timeline()` → `events()` (avoids `timeline.timeline()` after rename); test source + HTML pre-block aligned. Tests pass.
- **J14** (low ordering): folded into J3 — @ReadOnly is now §2.4, comes after the simpler transaction lifecycle in §2.2.
- **J15** (low userfriendly): §1.3 maven prose expanded — the xbean/asm pair is now spelled out as "the default entity scanner reads @Entity annotations directly off .class bytecode rather than calling Class.forName … xbean-finder builds an annotation index over the classpath roots; asm is its bytecode reader." Plus pointer to the EntityScanner SPI for swapping.
- **J16** (low names): §2.1 list entries for `persistenceUnits` / `persistenceUnitName` rewritten — the plural form is explicitly the "discovery filter at boot time", the singular form picks "one of the booted PUs to open eagerly when a transaction begins". A "*Easy to confuse:*" callout follows the bullets.

## 2026-05-22 — docs/core.html (pending C1/C4/C7)
