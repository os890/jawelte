...
---
## 2026-05-18 — quarkus-full-poc: scope-module Quarkus extension; scope-01 green under @QuarkusTest

scope-module is now a real Quarkus extension. Added
`modules/scope-module/deployment/` as the deployment artifact and
wired the runtime side via the standard
`META-INF/quarkus-extension.properties` + `quarkus-extension.yaml`
metadata in `scope-module/impl`.

`ScopeModuleProcessor` ships two `@BuildStep` methods:

- **`registerScopes(ContextRegistrationPhaseBuildItem)`** returns a
  `ContextConfiguratorBuildItem` plugging `@TestClassScoped` and
  `@TestMethodScoped` into ArC's build-time pipeline via
  `ContextCreator`s that read the lazy-initialised
  `TestScopeCurrentStores` slots.
- **`registerScopedBeansAsBeans(CombinedIndexBuildItem)`** returns
  an `AdditionalBeanBuildItem` listing every class in the
  application's Jandex annotated with one of the two scopes —
  closes the "Quarkus's annotated discovery doesn't pick up the
  test class's static-nested types" gap that was causing
  cdi-module's auto-mock BCE to mock real user beans.

Wiring discoveries the hard way:

- `META-INF/quarkus-extension.properties` needs `deployment-artifact=`
  (escaped colons) — Quarkus's bootstrap reads this exact key.
- `META-INF/quarkus-build-steps.list` is auto-generated by Quarkus's
  `quarkus-extension-processor` annotation processor — added it as
  an `<annotationProcessorPaths>` entry in the deployment pom.
  Without that file, Quarkus's bootstrap silently doesn't see the
  build steps at all.
- The pre-existing `JaweltAutoMockBuildCompatibleExtension` also
  needed a qualifier-set normalisation fix: ArC tags every bean's
  qualifier set with `@Default` + `@Any`, while IPs only carry the
  qualifiers the user wrote — stripping both implicit qualifiers
  from both sides before comparing now lets a real bean with
  `[Default, Any]` satisfy an IP declared as `[Default]`.

`TestScopeCurrentStores` now lazy-initialises its slots on first
`methodStore()` / `classStore()` access (under @QuarkusTest no
`ArcContextContributor` runs to pre-populate them) and exposes a
`reset()` method the lifecycle adapter calls in `afterAll`.
`ScopeLifecycleAdapter` reads the stores via a fallback chain:
`TestContext` metadata first (standalone-ArC contributor path),
then the static holder (`@QuarkusTest` path) — keeping the existing
`quarkus-poc` path green.

Score under `@QuarkusTest`:
