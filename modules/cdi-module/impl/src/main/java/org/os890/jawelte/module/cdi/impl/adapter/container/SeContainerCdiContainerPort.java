...
public class SeContainerCdiContainerPort implements CdiContainerPort {
     ...
     *
     * <p>Portable API only, and it does not currently succeed on either
     * runtime the suite uses. CDI gives exactly one handle on a running
     * container — {@link SeContainer} — and no way at all to abandon a
     * bootstrap that threw: {@code initialize()} either returns the
     * handle or loses it. Measured on a failed start, OpenWebBeans hands
     * out an {@code OwbCDI} (not a {@code SeContainer}, no shutdown),
     * and Weld's is not reachable as a running {@code SeContainer} at
     * that point either — so both fall through and the caller reports a
     * collateral failure instead.
     *
