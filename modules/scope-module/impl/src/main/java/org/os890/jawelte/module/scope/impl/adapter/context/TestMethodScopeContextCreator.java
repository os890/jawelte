...
/**
 * ArC {@link ContextCreator} for {@code @TestMethodScoped}. Invoked
 * once per {@code Arc.initialize}; reads the per-test-class store
 * from {@link TestScopeCurrentStores#methodStore()} (set by
 * {@code ScopeArcContextContributor} during the preceding
 * {@code beforeAll}) and returns a {@link TestMethodScopedContext}
 * delegating to it.
 *
 ...
 */
public class TestMethodScopeContextCreator implements ContextCreator {

    /** No-arg constructor required by ArC's reflective creator lookup. */
    public TestMethodScopeContextCreator() {
    }

    @Override
    public InjectableContext create(Map<String, Object> params) {
        return new TestMethodScopedContext(TestScopeCurrentStores.methodStore());
    }
}
