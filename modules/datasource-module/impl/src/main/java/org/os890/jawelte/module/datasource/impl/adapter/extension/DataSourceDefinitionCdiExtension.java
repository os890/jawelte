...
public class DataSourceDefinitionCdiExtension implements Extension {
     ...
     * there is none to keep: what the container hands over depends on
     * scan timing. Where a stable order matters it is imposed by sorting
     * on the name, which is deterministic.
     */
    ...
    void onAfterBeanDiscovery(@Observes AfterBeanDiscovery event) {
        ...
        for (String name : definitionsByName.keySet().stream().sorted().toList()) {
     ...
     * declared name.
     *
     * @return an immutable snapshot, read by the lifecycle adapter
     */
