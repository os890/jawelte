 ...
 */
package org.os890.jawelte.module.testcontrol.impl.adapter.extension;
...
/**
 * CDI Extension shipped by testcontrol-module/impl. Single
 * responsibility: at {@code ProcessAnnotatedType} time, every class
 * carrying the {@link ConfigBean} stereotype has its effective scope
 * remapped from {@link ApplicationScoped} (the scope contributed by
 * the {@code @ConfigBean} stereotype) to {@link TestClassScoped}, so
 * that user configuration beans are recreated per test class instead
 * of once per CDI container.
 *
 * <p><b>Trigger surface.</b> The remap fires unconditionally when
 * testcontrol-module is on the classpath — it does <em>not</em>
 * require any test method to carry {@code @TestControl}. Per
 * TICKET-010 there is no opt-out for individual {@code @ConfigBean}
 * classes.
 *
 ...
 * registration shipped in this module. Instantiated once per
 * {@code SeContainer} startup.
 */
public class TestControlCdiExtension implements Extension {

    /** No-arg constructor required by the CDI runtime. */
    public TestControlCdiExtension() {
    }
