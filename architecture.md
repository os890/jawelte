...
**Deployment-phase ordering between datasource-module and jpa-module.** datasource-module builds and binds its data sources from an `AfterDeploymentValidation` observer at `@Priority(Interceptor.Priority.LIBRARY_BEFORE)` (1000); jpa-module builds its `EntityManagerFactory`s from one at `@Priority(2000)`, so a persistence unit naming a `<jta-data-source>` finds it already bound (#123). Both are explicit, because the lifecycle-adapter priorities (150 before 200) order the *test* lifecycle and say nothing about observer order inside a single CDI event — the distinction #122 turned on. Observer ordering *within one event type* is specified by CDI and holds on any runtime, unlike the cross-extension bean visibility #124 hinged on, which no priority can fix.

jpa-module resolves the name through jndi-module's writable root rather than through datasource-module, so it depends on one interface jar and not on datasource-module at all; any module that binds a data source resolves the same way, which is what a real container does with the element. A name nothing binds is not an error — the unit keeps jpa-module's own generated database, which is why `scenario-47`'s production-shaped `persistence.xml` naming `java:jboss/datasources/PostgresDS` still behaves as it always did. The rule is **resolve when declared, override when not**.
